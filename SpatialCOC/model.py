...
def _resolve_hidden_sizes(layer_sizes, architecture):
    """Return the two hidden widths and latent width used by COC."""
    latent_size = layer_sizes[-1]
    if architecture == "legacy_fixed":
        return 512, 256, latent_size
    if len(layer_sizes) == 2:
        return layer_sizes[0], layer_sizes[0], latent_size
    return layer_sizes[0], layer_sizes[1], latent_size
...
class EncoderDecoderNet(nn.Module):
    ...
    decoder : Decoder
        The decoder network.
    """
    def __init__(self, input_size, layer_sizes, architecture="tutorial"):
        super(EncoderDecoderNet, self).__init__()
        hidden_size1, hidden_size2, latent_size = _resolve_hidden_sizes(layer_sizes, architecture)
        self.encoder = Encoder(input_size, hidden_size1, hidden_size2, latent_size)
        self.decoder = Decoder(latent_size, hidden_size1, hidden_size2, input_size)
...
class cca_loss():
    ...
    def loss(self, H1, H2):
        ...
        SigmaHat11 = (1.0 / (m - 1)) * torch.matmul(H1bar, H1bar.t()) + \
                     r1 * torch.eye(o1, device=device, dtype=H1.dtype)
        SigmaHat22 = (1.0 / (m - 1)) * torch.matmul(H2bar, H2bar.t()) + \
                     r2 * torch.eye(o2, device=device, dtype=H2.dtype)
...
class DeepPairedNetworks(nn.Module):
    ...
    def __init__(self, layer_sizes1, layer_sizes2, input_size1, input_size2,
                 n_components, use_all_singular_values, architecture="tutorial",
                 device=torch.device('cpu')):
        super(DeepPairedNetworks, self).__init__()
        self.model1_ = EncoderDecoderNet(input_size1, layer_sizes1, architecture).double()
        self.model2_ = EncoderDecoderNet(input_size2, layer_sizes2, architecture).double()
...
class COC(BaseEmbed):
    """
    Cross-omics correction (COC) class.

    This class learns representations that capture the correlation between two modalities of the data.
    ...
    def __init__(
            ...
            use_all_singular_values=False, device=None,
            epoch_num=200, batch_size=800, learning_rate=1e-3, reg_par=1e-5,
            tolerance=1e-5, print_train_log_info=True, random_state=2024,
        ...
        self._valid_inputs(input_size1, input_size2, n_components,
                           ...
                           tolerance, print_train_log_info, architecture)
        ...
        self.device_ = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") if device is None else torch.device(device)
    ...
    def fit(self, Xs, y=None):
        """
        Fits the COC model to the given data.
        ...
        pbar = tqdm(total=self.epoch_num, desc="Training Progress", disable=not self.print_train_log_info)
        sampler_kwargs = {}
        if self.random_state_ is not None:
            sampler_generator = torch.Generator()
            sampler_generator.manual_seed(int(self.random_state_))
            sampler_kwargs["generator"] = sampler_generator

        # Training loop
        for epoch in range(self.epoch_num):
            self.model_.train()  # Set the model to training mode
            batch_idxs = list(BatchSampler(RandomSampler(range(data_size), **sampler_kwargs),
    ...
    def _valid_inputs(self, input_size1, input_size2, n_components,
        ...
        if architecture not in {"tutorial", "legacy_fixed"}:
            raise ValueError("architecture must be either 'tutorial' or 'legacy_fixed'")
